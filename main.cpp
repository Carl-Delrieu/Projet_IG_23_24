...
MiniMesh::MiniMesh( const std::vector<Vertex>& v, const std::vector<Face>& f ) :
    ...
    }
    {
        ...
        vertexBinding->setBuffer(
            m_gpuVertices.get(), offsetof( Vertex, weights[0] ), sizeof( Vertex ) );
        vertexBinding->setFormat( 1, GL_FLOAT, GL_TRUE );
        m_vao->enable( 2 );
    }
    {
        ...
        vertexBinding->setBuffer(
            m_gpuVertices.get(), offsetof( Vertex, weights[1] ), sizeof( Vertex ) );
        vertexBinding->setFormat( 1, GL_FLOAT, GL_TRUE );
        m_vao->enable( 3 );
    }
...
std::unique_ptr<MiniMesh> makeCylinder( vec3 base    = vec3( 0, 0, 0 ),
    ...
    for ( int i = 0; i < subdiv2; i++ )
    {
        ...
        for ( int j = 0; j < subdiv1; j++ )
        {
            ...
            nv.weights[1] = 1.0f - nv.weights[0];
...
void initialize() {
    ...
    g_vertexShaderSource = globjects::Shader::sourceFromFile( dataPath + (CPU_or_GPU ? "./gpu.vert" : "./basic.vert") );
...
void draw() {
    ...
    if (CPU_or_GPU == GPU) {
        g_program->setUniform("fixed_bone", fixedBone.transformation);
        g_program->setUniform("mobile_bone", mobileBone.transformation);
    }
...
void moveBones(float angle) {

    mat4 transformation_tmp = glm::rotate(mobileBone.transformation, angle, vec3(0.0f, 0.0f, 1.0f));
    mobileBone.transformation = transformation_tmp;

    if (CPU_or_GPU == CPU) {
        vec4 point_tmp;
        for (int i = 0; i < g_mesh->vertices().size(); i++) {
            point_tmp = vec4(g_mesh->restVertices()[i].pos, 1.0f);
            point_tmp = (fixedBone.transformation * mobileBone.transformation * g_mesh->restVertices()[i].weights[0] + fixedBone.transformation * g_mesh->restVertices()[i].weights[1]) * point_tmp;
            g_mesh->vertices()[i].pos = vec3(point_tmp.x, point_tmp.y, point_tmp.z) / point_tmp.w;
        }
    }
}
...
int main( int argc, char *argv[] ) {
    ...
    std::cout << std::endl
              ...
              << "CPU_or GPU :     " << (CPU_or_GPU ? "GPU" : "CPU") << std::endl;
